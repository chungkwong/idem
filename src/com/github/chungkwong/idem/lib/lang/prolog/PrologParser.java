...
public class PrologParser implements SimpleIterator<Predication>{
	...
	public PrologParser(PrologLex lex){
		this(lex,OperatorTable.DEFAULT_OPERATOR_TABLE);
	}
	/**
	 * Construct a PrologParser
	 * @param lex The provider of tokens
	 * @param table operator table
	 */
	public PrologParser(PrologLex lex,OperatorTable table){
	...
	class ParseState{
		...
		boolean pushOperator(String name,PushBackIterator lex){
			if(expected==ExpectClass.INFIX_OR_POSTFIX){
				Operator infix=table.getInfixOperators().get(name);
				Operator postfix=table.getPostfixOperators().get(name);
				if(infix==null){
					if(postfix==null)
					...
					if(peek instanceof String&&(table.getInfixOperators().containsKey((String)peek)
							||table.getPostfixOperators().containsKey((String)peek))){
						pushOperator(postfix);
					}else{
						pushOperator(infix);
					}
				...
				if(lex.hasNext()&&lex.peek().equals("(")){
					...
					Operator prefix=table.getPrefixOperators().get(name);
					if(prefix==null){
		...
		private List<Term> convertTermtoList(Term t,boolean multi){
			if(multi){
