...
public class AtomConcat extends BuildinPredicate{
	...
	public boolean activate(List<Term> argments,Processor exec){
		Term atom1=argments.get(0),atom2=argments.get(1),atom12=argments.get(2);
		if(atom12 instanceof Variable){
			if(atom1 instanceof Variable||atom2 instanceof Variable)
				...
				return atom12.unities(new Atom(((Atom)atom1).getValue().toString()+((Atom)atom2).getValue().toString()),exec.getCurrentSubst());
			}
			...
			if(isAtom(atom1)){
				if(isAtom(atom2))
					...
					return atom12.toString().startsWith(atom1.toString())
							&&atom2.unities(new Atom(atom12.toString().substring(atom1.toString().length())),exec.getCurrentSubst());
				}else
					throw new TypeException("atom",atom2);
			}else if(atom1 instanceof Variable){
				if(isAtom(atom2))
					return atom12.toString().endsWith(atom2.toString())
							&&atom1.unities(new Atom(atom12.toString().substring(0,atom1.toString().length())),exec.getCurrentSubst());
				else if(atom2 instanceof Variable){
					//FIXME
					return atom1.unities(atom12,exec.getCurrentSubst())&&atom2.unities(new Atom(""),exec.getCurrentSubst());
				}else
					throw new TypeException("atom",atom2);
			}else{
				throw new TypeException("atom",atom1);
	...
	private static boolean isAtom(Term t){
		return t instanceof Atom&&((Atom)t).getValue()instanceof String;
	}
}