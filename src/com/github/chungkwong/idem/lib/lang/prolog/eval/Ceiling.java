 ...
 */
public class Ceiling extends Evaluable{
	public static final Ceiling INSTANCE=new Ceiling();
	private Ceiling(){
		super(new EvaluableFunctor("ceiling",1));
	}
	@Override
	protected Term evaluate(Object[] args){
		if(args[0] instanceof BigInteger)
			...
			return new Constant(arg.round(new MathContext(Math.max(arg.precision()-arg.scale(),0),RoundingMode.CEILING)).toBigInteger());
		}else
